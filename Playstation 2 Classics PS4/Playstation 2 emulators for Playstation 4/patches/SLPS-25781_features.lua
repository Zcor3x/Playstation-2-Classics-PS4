-- Lua 5.3
-- Title: Fu'un Super Combo - SLPS-25839 (Japan) v1.02
...
local LH1 =	-- update splash screen
	function()
		local splashNum = eeObj.GetGpr(gpr.a0)
		if splashNum == 4 then
...
local FH2A =	-- prepare Display Settings menu
	function()
		local artwork = "\xef\xbc\xa1\xef\xbc\xb2\xef\xbc\xb4\xef\xbc\xb7\xef\xbc\xaf\xef\xbc\xb2\xef\xbc\xab"
		local scanlines = "\xef\xbc\xb3\xef\xbc\xa3\xef\xbc\xa1\xef\xbc\xae\xef\xbc\xac\xef\xbc\xa9\xef\xbc\xae\xef\xbc\xa5\xef\xbc\xb3"

		eeObj.WriteMemStrZ(0x2bad40, artwork)
		eeObj.WriteMemStrZ(0x2bad5f, scanlines)
	end
...
local FH2B =	-- update Position X (now Artwork) description
	function()
		...
		local pos = asSigned(eeObj.ReadMem32(base + 192))
		-- limit to valid range
		pos = pos % 3
		eeObj.WriteMem32(base + 192, pos)
...
local FH2C =	-- update Position Y (now Scanlines) description
	function()
		...
		local pos = asSigned(eeObj.ReadMem32(base + 196))
		-- limit to valid range
		pos = pos % 2
		eeObj.WriteMem32(base + 196, pos)
...
local CHKDATA_MAINELF	= 0x3c020034
local CHKDATA_MOVIEELF	= 0x3c020014

local elfChkMain = function(opcode, pc, expectedOpcode)
	local chkData = eeObj.ReadMem32(0x10011c)

	if chkData == CHKDATA_MAINELF then
...
local elfChkMovie = function(opcode, pc, expectedOpcode)
	local chkData = eeObj.ReadMem32(0x10011c)

	if chkData == CHKDATA_MOVIEELF then
		assert(opcode == expectedOpcode, string.format("Overlay opcode mismatch @ 0x%06x: expected 0x%08x, found %08x", pc, expectedOpcode, opcode))
		return true
	else
		return false
...
local hooks = {
	-- Loading screen
	eeObj.AddHook(0x1071d8, function(op, pc) return elfChkMain(op, pc, 0x27bdfff0) end, TH1),	-- <__0oNLoadingScreenctv>:
	eeObj.AddHook(0x1072c0, function(op, pc) return elfChkMain(op, pc, 0x27bdffd0) end, TH2),	-- <__0oNLoadingScreendtv>:

	-- SNK logo
	eeObj.AddHook(0x1011b0, function(op, pc) return elfChkMovie(op, pc, 0x8e04000c) end, LH1),	-- <__0f5X__N_8_logo_cpp_6df69e7dILogoPlayEstepv>:

	-- turn on auto save
	eeObj.AddHook(0x10c4c8, function(op, pc) return elfChkMain(op, pc, 0xa0800008) end, FH1),	-- <__0o5LMenuContentISaveLoadctv>:

	-- patch Position x/y options to show Artwork/Scanlines options
	eeObj.AddHook(0x118c78, function(op, pc) return elfChkMain(op, pc, 0x34050005) end, FH2A),	-- <__0o5X__N_8_menu_cpp_761e2d17TMenuDispSettingItemctv>:
	eeObj.AddHook(0x118ab0, function(op, pc) return elfChkMain(op, pc, 0x02802825) end, FH2B),	-- <__0f5X__N_8_menu_cpp_761e2d17XMenuDispSettingPropertyEstepv>:
	eeObj.AddHook(0x118aec, function(op, pc) return elfChkMain(op, pc, 0x02802825) end, FH2C),	-- <__0f5X__N_8_menu_cpp_761e2d17XMenuDispSettingPropertyEstepv>:

	-- apply artwork / scanlines settings
	eeObj.AddHook(0x10c224, function(op, pc) return elfChkMain(op, pc, 0x8e0500c4) end, FH3),	-- <__0FTapplyDisplaySettingv>:
}
...
--		onHIDPadEvent(0, true, PadConnectType.HID)
		...
		-- bug report:
		-- Could you possible decrease to 31.62% from the current volume?
		-- Fu’un Mokujiroku: -10.97LKFS
		-- Fu’un Super Tag Battle: -9.97LKFS
