-- Lua 5.3
-- Title:   Dark Chronicle PS2 - SCES-51190 (Europe) v1.00
...
local function getGameSaveData()
	...
	return eeObj.ReadMem32(gp - 30084)
end
...
-- register hooks
local hook1  = eeObj.AddHook(0x2aef10, 0x27bdfeb0, H1)	-- <MenuChapterInit(mgCMemory *, int *, int, int)>:
local hook2  = eeObj.AddHook(0x19f4e4, 0xffbf0050, H2)	-- <CUserDataManager::GetItem(int, int)>:
local hook3a = eeObj.AddHook(0x2feb30, 0xdfbf0040, H3A)	-- <CDngFloorManager::IsClearMostFastDestroy(void)>:
local hook3b = eeObj.AddHook(0x2fee54, 0x0040202d, H3B)	-- <CDngFloorManager::IsClearPractice(int)>:
local hook3c = eeObj.AddHook(0x2ffaa8, 0x0040202d, H3C)	-- <CheckFishingRecord(float)>:
local hook3d = eeObj.AddHook(0x280ed8, 0x0200202d, H3D)	-- <_ADD_YARIKOMI_MEDAL(RS_STACKDATA *, int)>:
local hook4  = eeObj.AddHook(0x2007e4, 0x8e220004, H4)	-- <CInventUserData::LevelCheck(USER_PICTURE_INFO *)>:
local hook5  = eeObj.AddHook(0x205ed0, 0x0040202d, H5)	-- <CMenuInvent::IsMakeObject(int, int)>:
local hook6  = eeObj.AddHook(0x1e0670, 0xae031208, H6)	-- <CMonsterMan::CheckDamage(void)>:
local hook7a = eeObj.AddHook(0x1a1518, 0xe4620000, H7)	-- <CBattleCharaInfo::AddHp_Point(float, float)>:
local hook7b = eeObj.AddHook(0x1a2124, 0xe4c20000, H7)	-- <CBattleCharaInfo::Step(void)>:
local hook8  = eeObj.AddHook(0x295988, 0x44900000, H8)	-- <CShop::AddMoney(int)>
local hook9  = eeObj.AddHook(0x200820, 0x0005082a, H9)	-- <CInventUserData::SetCreateItemFlag(int, int)>:
local hook10 = eeObj.AddHook(0x306f90, 0x0260282d, H10)	-- <InitSuccess(CScene *)>:
local hook11 = eeObj.AddHook(0x20ed80, 0x0000302d, H11)	-- <GetChildFishNo(int, int)>:
local hook12 = eeObj.AddHook(0x23ba18, 0x0040902d, H12)	-- <CBaseMenuClass::IsSpectolFusion(int, int)>:
local hook13 = eeObj.AddHook(0x244d4c, 0x8e93001c, H13)	-- <CMenuItemInfo::IsAskExtend(int, int)>:
local hook14 = eeObj.AddHook(0x31fed8, 0xa0650001, H14)	-- <CQuestData::QuestClear(int)>:
local hook15 = eeObj.AddHook(0x311c34, 0xa0430005, H15)	-- <CNameRegiMenu::KeyStep(void)>:
local hook16a= eeObj.AddHook(0x2ae4c0, 0x27bdff90, H16A)-- <CEditData::Analize(int, int *, int *)>:
local hook16b= eeObj.AddHook(0x2ae508, 0x80840000, H16B)-- <CEditData::Analize(int, int *, int *)>:
local hook16c= eeObj.AddHook(0x2ae580, 0x7bb00000, H16C)-- <CEditData::Analize(int, int *, int *)>:
local hook17 = eeObj.AddHook(0x297088, 0x02e0202d, H17)	-- <CShopMenu::KeyStep(void)>:
local hook18 = eeObj.AddHook(0x19c214, 0x2625ffff, H18)	-- <CMonsterBox::EnableChange(int)>:
local hook19 = eeObj.AddHook(0x2bc248, 0x24420001, H19)	-- <CMenuMosSelect::KeyStep(void)>:
local hook20a= eeObj.AddHook(0x2bb9a0, 0x24020003, H20A)-- <CMenuMosSelect::KeyStep(void)>:
local hook20b= eeObj.AddHook(0x1d2e18, 0x0040902d, H20B)-- <DngMainKey(void)>:
local hook21 = eeObj.AddHook(0x27a1c0, 0x27bdffc0, H21)	-- <_SPHIDA_GET_PRIZE(RS_STACKDATA *, int)>:
local hook22 = eeObj.AddHook(0x21b55c, 0x34630020, H22)	-- <SetGyoRaceRanking(int)>:



-- cheats; disable for release
-- enable debug functions (see docs)
--local dbg1a = eeObj.AddHook(0x14b270, 0x8e020460, DH1a)	-- <CGamePad::UpDate(void)>:
--local dbg1b = eeObj.AddHook(0x14b8c4, 0x8c820460, DH1b)	-- <CGamePad::On2(int)>:
--local dbg1c = eeObj.AddHook(0x14b944, 0x8c820460, DH1c)	-- <CGamePad::Down2(int)>:
-- invincibility
--local cheat1 = eeObj.AddHook(0x1a1490, 0x8c820074, CH1)	-- <CBattleCharaInfo::AddHp_Point(float, float)>:
-- infinite power (ridepod)
--local cheat2a = eeObj.AddHook(0x1a20f4, 0x46010001, CH2)-- <CBattleCharaInfo::Step(void)>
--local cheat2b = eeObj.AddHook(0x1a2104, 0x46010001, CH2)-- <CBattleCharaInfo::Step(void)>
-- one hit kill
--local cheat3 = eeObj.AddHook(0x1e013c, 0x00621823, CH3)	-- <CMonsterMan::CheckDamage(void)>:
-- always have full map
--local cheat4 = eeObj.AddHook(0x19293c, 0x0040202d, CH4)	-- <MainLoop(void)>:
